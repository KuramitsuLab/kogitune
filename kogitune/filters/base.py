...
import json
# import os
from ..adhoc_args import verbose_print, adhoc_log, save_adhoc_log
...
class TextFilter(object):
    ...
    def _from_jsonl_single(self, filenames: str, N=-1, output_path=None):
        ...
        adhoc_log('filter', 'input_files', filenames)
        adhoc_log('filter', 'filter_config', self.as_json())
        w = None
        ...
        if output_path:
            ...
            verbose_print(f'完了//Complete: {newpath} {c}/{n} {c/n:.3f}')
            adhoc_log('filter', 'output_file', newpath)
            adhoc_log('filter', 'total', n)
            adhoc_log('filter', 'filtered', c)
            save_adhoc_log(newpath)
